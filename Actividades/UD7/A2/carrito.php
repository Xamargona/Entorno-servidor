...
<body>
    <?php
        ...
        foreach ($_SESSION['carrito'] as $producto => $cantidad) {
            ...
            $precioproducto = 0;
            $precioproducto = round(($productos['precio']*$message['valordivisa']),2);
            if ($productos['oferta'] != 0) {
                $precioproducto = round(($productos['precio']*$message['valordivisa']),2)-(round(($productos['precio']*$message['valordivisa']),2)/100*$productos['oferta']);
            ...
            echo '<div id="carritoproducto">
                    ...
                    <p>'.$productos['nombre'].' - '.$cantidad['cantidad'].' '.$message['unidades'].': '.$precioproducto.$message['divisa'].'/'.$message['unidad'].'</p>
                </div>';
            $precio += ($precioproducto * $cantidad['cantidad']);
        }
        echo '<p>'.$message['preciototal'].': '.$precio.$message['divisa'].'</p>';
        unset($conexion);
        unset($resultado);
    ?>
</body>
