<?php
    ...
    if (!empty($_POST)) {
        ...
        if (empty($_FILES['foto'])) {
            ...
            // Comprobamos que el DNI sea válido y que no hayan problemas al mover el archivo
            if (!isset($error)) {
                //Nombre que equivale a su vez a la ruta
                ...
                if (!move_uploaded_file($_FILES['foto']['tmp_name'], $nombre_foto)) {
                    ...
                    // Seleccionamos, escalamos y guardamos la imagen
                    $imagen_original = imagecreatefrompng($nombre_foto);
                    $imagen_alterada = imagescale($imagen_original, imagesx($imagen_original)*0.75);
                    imagepng($imagen_alterada, $_POST['dni'].'-mini.png');
                }
            } else {
